...
docker stop dk_manager; docker rm dk_manager; docker run -d -it --name dk_manager --network host $LOG_LIMIT_PARAM $DOCKER_SHARE_PARAM  -v $HOME_DIR/.dk:/app/.dk --restart unless-stopped -e USER=$DK_USER -e DOCKER_HUB_NAMESPACE=$DOCKER_HUB_NAMESPACE -e ARCH=$ARCH $DOCKER_HUB_NAMESPACE/dk_manager:latest
...
if [[ "$dk_ivi_value" == "true" ]]; then
    ...
    if [ -f "/etc/nv_tegra_release" ]; then
        ...
        if docker run --help | grep -q -- '--gpus'; then
            echo "Docker supports --gpus option"
            # You can add further commands here that require --gpus
            docker stop dk_ivi; docker rm dk_ivi ; docker run -d -it --name dk_ivi -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=:0 -e  XDG_RUNTIME_DIR=$XDG_RUNTIME_DIR --restart unless-stopped --network host $LOG_LIMIT_PARAM $DOCKER_SHARE_PARAM -v $HOME_DIR/.dk:/app/.dk -e DKCODE=dreamKIT -e DK_USER=$DK_USER -e DK_DOCKER_HUB_NAMESPACE=$DOCKER_HUB_NAMESPACE -e DK_ARCH=$ARCH --gpus all --privileged -e DK_CONTAINER_ROOT="/app/.dk/" -e DK_VIP=$dk_vip_demo $DOCKER_HUB_NAMESPACE/dk_ivi:latest
        else
            echo "Docker does NOT support --gpus option"
            # Fallback or error handling here
            docker stop dk_ivi; docker rm dk_ivi ; docker run -d -it --name dk_ivi -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=:0 -e  XDG_RUNTIME_DIR=$XDG_RUNTIME_DIR -e QT_QUICK_BACKEND=software --restart unless-stopped --network host $LOG_LIMIT_PARAM $DOCKER_SHARE_PARAM -v    $HOME_DIR/.dk:/app/.dk -e DKCODE=dreamKIT -e DK_USER=$DK_USER -e DK_DOCKER_HUB_NAMESPACE=$DOCKER_HUB_NAMESPACE -e DK_VIP=$dk_vip_demo -e DK_ARCH=$ARCH -e DK_CONTAINER_ROOT="/app/.dk/" $DOCKER_HUB_NAMESPACE/dk_ivi:latest
        fi
    else
